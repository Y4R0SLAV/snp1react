...
export const Header = () => {
  ...
  return <header className={cx({wrapperFixed: headerMoving})}>
      <div className={globalStyles._container}>
        <nav className={s.nav}>
          ...
          <div className={s.btns}>
            {buttons.map(item => <BlockPNormal key={item.id}> <a href={item.href} rel="noreferrer" className={s.btn} target="_blank"> {item.title} </a> </BlockPNormal>)}
          </div>

          <BlockPNormal> <a className={s.number} href="tel:+79999999999"> +7 999 999 99 99</a> </BlockPNormal>
        </nav>
    </div>
  </header>
}